...
public class EnemyChargerScript : MonoBehaviour
{
    ...
    bool lastRun, lastattack, lastfein, lastgrounded;
    ...
    public void setAnim(bool run, bool attack , bool fein)
    {
        if (lastRun != run)
        {
            animator.SetBool("shouldRun", run);
            lastRun = run;
        }
        if (lastattack != attack)
        {
            animator.SetBool("shouldAttack", attack);
            lastattack = attack;
        }
        if (lastfein != fein)
        {
            animator.SetBool("shouldFein", fein);
            lastfein = fein;
        }
    }
